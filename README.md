...
1. 当前目录不是 git 仓库，所以计划中的分步提交未执行。
2. Worker 真实消费链仍依赖 Redis、真实 `LLM_API_KEY`、真实 ComfyUI 或第三方 API 可访问。
3. ComfyUI 工作流还是通用结构，未绑定具体节点模板。
4. 第三方图像 API 当前是通用适配层，具体请求体和响应体可能还需按目标供应商细化。
5. 当前还没有按后端区分更细的成本模型。
